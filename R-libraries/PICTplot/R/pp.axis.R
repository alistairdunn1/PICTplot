...
#'
#' @description  Wrapper function for the R function axis to add an axis to a plot generated by PICT, allowing full freedom to format an axis and tick mark labels.
#' @param side integer specifying which side of the plot the axis is drawn on.
#' @param at the points (in degrees) at which tickmarks are to be drawn. If NULL (default) then the ticks will placed at 'prettied' degree points and, if labels = TRUE, labels are attached to whole degrees.
#' @param labels either a character or expression vector of labels to be placed at the tickpoints, or a logical value specifying whether no labels are written or whether whole degree annotations are to be made at the appropriate tickmarks. If labels is not logical, at should also be supplied and of the same length. If FALSE no labels are written.
#' @param ...  other arguments to be passed to the R graphics package function axis including parameters passed by ... in the axis function. E.g. tick to determine if tickmarks and the axis are both drawn, tcl for tickmark length, col and tick.col for the colour of the axis and tickmarks, mgp to position the axes and the labels.
#' @return A vector of numeric locations on the axis scale at which tick marks were drawn. Side effect is to add an axis to the existing plot.
#; @details If at is NULL, If labels = TRUE (default), only the ticks which are whole numbers of degrees are labelled, in which case labels will include the degree symbol and the initial label on the axis will also include the appropriate hemisphere symbol (E, W, N, S).
#' @export
#'
"pp.axis" <-
