...
#' @param radius a vector of circle radii which are interpreted as the relative sizes of the radii. If scale is NULL then the radii are interpreted as fractions of the x-axis length. If scale is specified then the radii are interpreted as relative sizes and need not be normalised.
#' @param scale scales the circles so that the circle of largest radius is the proportion of the length of the x axis specified by scale. Default value is NULL, which means radius values are interprested as fractions of the x-axis.
#' @param plot.centre logical determining if a point is to be plotted at the centre of each circle.
...
#' @param border colour of circle border and centre point. The default is to use par("fg"). Use border = NA to omit borders. (See the R function polygon for more information about border, col, density, and angle)
#' @param col solid fill colour for circle. If col = NA circle is not filled. If density is positive then col sets the colour of the stripe lines.
#' @param density density of stripe lines for filling the circle. Use density = NULL to omit stripes but allow solid fill.
#' @param angle angle of fill stripe lines
#' @param ... other plotting parameters to pass to the polygon and points functions, e.g. lty for ploygon border, pch and cex for circle centre points.
